...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            # Read video 处理视频
            self.mode = 'video'
            ret_val, img0 = self.cap.read() #按帧读取视频；ret_val:如果读取帧正常返回true，读到结尾返回false;img0是每一帧图像
            if not ret_val:
                self.count += 1
                self.cap.release() #释放摄像头 每一个完整的文件检测完毕后都要释放
    ...
    def new_video(self, path):
        self.frame = 0
        self.cap = cv2.VideoCapture(path) #如果参数是0则是打开摄像头；如果是路径则打开本地视频
        self.nframes = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT)) #7为视频文件中的帧数
...
#
# class LoadStreams(Ui_CameraPage):  # multiple IP or RTSP cameras #处理摄像头？
#     def __init__(self, sources='streams.txt', img_size=416, half=False):
#         super(LoadStreams, self).__init__(Ui_CameraPage)
#         self.mode = 'images'
#         self.img_size = img_size
#         self.half = half  # half precision fp16 images
#
...
class LoadStreams:  # multiple IP or RTSP cameras #处理摄像头？
    def __init__(self, sources='streams.txt', img_size=416, half=False):
        self.mode = 'images'
        self.img_size = img_size
        self.half = half  # half precision fp16 images
        ...
        for i, s in enumerate(sources):
            ...
            self.thread = threading.Thread(target=self.update, args=([i,cap,self.event]))
            self.event.set()
            print(' success (%gx%g at %.2f FPS).' % (w, h, fps))
            self.thread.start()
        print('')  # newline
    ...
    def update(self, index, cap, event):
        # Read next stream frame in a daemon thread
        n = 0
        while event.isSet():
            #while cap.isOpened():
    ...
    def __next__(self):
       while self.event.isSet():
           ...
           return self.sources, img, img0, None, self.event
...
class LoadImagesTest:
    def __init__(self, path, img_size=416, half=False): #初始化构造函数
        path = str(Path(path))
        files = []
        if os.path.isdir(path):
            files = sorted(glob.glob(os.path.join(path, '*.*'))) #遍历目录，将目录和文件名合成一个路径
        elif os.path.isfile(path):
            files = [path]

        images = [x for x in files if os.path.splitext(x)[-1].lower() in img_formats]  #遍历files，分割出后缀名检验是否是图片
        videos = [x for x in files if os.path.splitext(x)[-1].lower() in vid_formats]  #文件还是视频文件，放入不同列表
        nI, nV = len(images), len(videos)

        self.img_size = img_size
        self.files = images + videos #更新files列表，将不符合要求的文件去掉，并且文件格式是先图片后视频
        self.nF = nI + nV  #加载的文件数量
        self.video_flag = [False] * nI + [True] * nV #将每一个文件加上是否是视频文件的flag
        self.mode = 'images'
        self.half = half  # 是否是半精度的fp16图片
        if any(videos):
            self.new_video(videos[0])  # 第一个视频文件开始读取，好构造迭代器迭代
        else:
            self.cap = None
        assert self.nF > 0, 'No images or videos found in ' + path
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            #从viedo_flag标志位判定为视频文件，开始逐帧提取
            self.mode = 'video'
            ret_val, img0 = self.cap.read() #read()读取当前帧，正确读取返回ret_val为true，imgo为帧图像数据，结束ret_val为flase
            if not ret_val: #一个视频文件读取结束，进行下一个文件的读取
                self.count += 1
                self.cap.release()
                if self.count == self.nF:  # 若为最后一个文件，停止读取
                    raise StopIteration
                else:
                    path = self.files[self.count]
                    self.new_video(path)
            ...
            # Read image
            self.count += 1
            img0 = cv2.imread(path)  # 按照BGR通道顺序读取图像
            assert img0 is not None, 'Image Not Found ' + path
            print('image %g/%g %s: ' % (self.count, self.nF, path), end='')
        ...
        # 转换图片
        img = img[:, :, ::-1].transpose(2, 0, 1)  # 将BGR 转换为 RGB, to 3x416x416
        img = np.ascontiguousarray(img, dtype=np.float16 if self.half else np.float32)  # uint8 to fp16/fp32
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
...
class LoadStreamsTest:  # 重构为调用本机摄像头
    # 可采用日志记录程序运行记录，重构为了节约运行时间，未采用记录
    def __init__(self, sources=0, img_size=416, half=False):
        # 初始化参数，sources没有输入默认为0，可改
        self.mode = 'images'
        self.img_size = img_size
        self.half = half  # half precision fp16 images
        ...
        assert cap.isOpened(), 'Failed to open local camera!'
        _, self.img = cap.read() # 读取摄像头第一帧，这里先重构为不采用多线程方式，可改
        flag = 0
        while cap.isOpened():
            flag += 1
            cap.grab() # 由于不是所有帧都进行处理，不用read方法，采用grab + retrieve方法节约时间
            if flag == 4:
                flag = 0
    ...
    def __next__(self):
        img0 = self.imgs.copy() # 拷贝，不受原数据改变的影响
        if cv2.waitKey(1) == ord('q'):  # q to quit
            cv2.destroyAllWindows()
            raise StopIteration

        # 以下为对图像进行初始化，无需较大更改，更改img0参数不为list
        # Letterbox
        img = [letterbox(img0, new_shape=self.img_size, interp=cv2.INTER_LINEAR)[0]]

        # Stack
        img = np.stack(img, 0)

        # Convert
        img = img[:, :, :, ::-1].transpose(0, 3, 1, 2)  # BGR - RGB,  3x416x416, uint8 - float32
        img = np.ascontiguousarray(img, dtype=np.float16 if self.half else np.float32)
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
