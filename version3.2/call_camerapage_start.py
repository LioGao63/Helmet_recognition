...
class CameraPageWindow(QWidget, Ui_CameraPage):
    ...
    def __init__(self, parent=None):
        ...
        # self.model = Darknet('cfg/yolov3_5.cfg', 416)
        # self.device = torch_utils.select_device(device='0')
        # self.model.load_state_dict(torch.load('weights/last.pt', map_location=self.device)['model'])
    ...
    def openImage(self):
        ...
        try:
            ...
            self.graphicsView.setText("打开文件失败，可能是文件类型错误")
    ...
    def openVideo(self):
        try:
            ...
            self.graphicsView.setText("打开视频失败，可能是文件类型错误")
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, _, cls in det:
                        ...
                        if cls_name == 'person':
                            ...
                            cropim0 = imForFace[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])]
                            if face_num<10:
                                face_num_str = "000%d" % face_num
                            elif face_num<100:
                                face_num_str = "00%d" % face_num
                            elif face_num<1000:
                                face_num_str = "0%d" % face_num
                            cv2.imwrite(face_capture_path + t_start + '/' + t1 + '-' + face_num_str + '.jpg', cropim0)
                            print("人脸图片区域已记录")
                            face_num += 1
...
class FaceCollectionWindow(QDialog, Ui_faceCollection):
    ...
    def save_event(self, faceImageName):
        ...
        try:
            ...
            print(filename)
            img = self.graphicsView.pixmap().toImage()
            # 保存到文件夹中
            img.save(filename)
