...
def create_modules(module_defs, img_size, arc):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            elif mdef['activation'] == 'swish':  #yolov3s.cfg中的激活函数 无用
        ...
        elif mdef['type'] == 'shortcut':  # nn.Sequential() placeholder for 'shortcut' layer 捷径层
            filters = output_filters[int(mdef['from'])]
            layer = int(mdef['from'])
            routs.extend([i + layer if layer < 0 else layer])

        elif mdef['type'] == 'reorg3d':  # yolov3-spp-pan-scale 没用到
            # torch.Size([16, 128, 104, 104])
            # torch.Size([16, 64, 208, 208]) <-- # stride 2 interpolate dimensions 2 and 3 to cat with prior layer
            pass

        elif mdef['type'] == 'yolo': # 输出三个维度feature map 三个map评分九个先验框
            yolo_index += 1
