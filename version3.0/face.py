    ...
    def show_camera(self, imageSource='0', save_txt=False, save_img=False):
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, _, cls in det:
                        ...
                        if cls_name == 'person':
                            ...
                            cv2.imwrite(face_capture_path + t_start + '/' + t1 + '-' + str(face_num) + '.jpg',
                                        cropim0)
                            print("人脸图片区域已记录")
                            face_num += 1
                ...
                # 结果
                if view_img:
                    show = cv2.resize(im0, (1080, 600))
                    show = cv2.cvtColor(show, cv2.COLOR_BGR2RGB)
                    showImage = QImage(show.data, show.shape[1], show.shape[0], QImage.Format_RGB888)
                    self.signal.emit(showImage)
...
class Image_thread(QThread,Util):
    ...
    def run(self):
        self.show_camera(self.fname)
        #self.updata.emit(str(fname))


class Video_thread(QThread,Util):
    signal = pyqtSignal(object)
...
class Camera_thread(QThread,Util):
    signal = pyqtSignal(object)
...
class CameraPageWindow(QWidget, Ui_CameraPage):
    ...
    #         print(fname)
    #         #self.cameraLabel.setPixmap(QPixmap(fname).scaled(self.cameraLabel.width(), self.cameraLabel.height()))
    #         self.show_camera(fname)
    #     except:
    #         self.cameraLabel.setText("打开文件失败，可能是文件内型错误")

    def openImage(self):
        try:
            fname, _ = QFileDialog.getOpenFileName(self, '打开文件', '/', 'Image files (*.jpg *.gif *.png)')
            self.thread = Image_thread(fname)  # 创建线程
    ...
    def openVideo(self):
        try:
            ...
            print(fname)
            #self.cameraLabel.setPixmap(QPixmap(self.show_camera(fname)))
            self.thread = Video_thread(fname)  # 创建线程
            self.thread.signal.connect(self.display)
            self.thread.start()  # 开始线程
        except:
            self.cameraLabel.setText("打开视频失败，可能是文件类型错误")
    ...
    def openCamera2(self):
        self.thread = Camera_thread()  # 创建线程
        self.thread.signal.connect(self.display)
        self.thread.start()  # 开始线程
...
class FaceCollectionWindow(QDialog,Ui_faceCollection):
    ...
    def openImage1(self):
        ...
        try:
            ...
            self.label.setPixmap(QPixmap(fname).scaled(self.label.width(), self.label.height()))
        except:
            self.label.setText("打开文件失败，可能是文件内型错误")

    def submit(self):
        print(fname)
        self.save_event(fname)

    def save_event(self,fname):
